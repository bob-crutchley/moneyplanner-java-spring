...
public class AuthController extends SecureController {
    ...
    public ResponseEntity<Model> login(@RequestBody String credentials) {
        ServiceResponse serviceResponse = authService.login(new Gson().fromJson(credentials, Credentials.class));
    ...
    public ResponseEntity<Model> logout(@RequestHeader(value = "Auth-Token") String authToken) {
        return authenticatedServiceRequest(authToken, (account) -> {
            ServiceResponse serviceResponse = authService.logout(account);
            return ResponseEntity.status(serviceResponse.getHttpStatus()).body(serviceResponse.getModel());
        });
    }
}
